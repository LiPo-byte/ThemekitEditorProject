...
def test_create_user_existing_username(
    ...
    # Same email triggers the email-conflict branch first.
    data = {"email": email, "username": random_username(), "password": password}
    r = client.post(
        f"{settings.API_V1_STR}/users/",
        headers=superuser_token_headers,
        json=data,
...
def test_create_user_existing_username_only(
    client: TestClient, superuser_token_headers: dict[str, str], db: Session
) -> None:
    email = random_email()
    username = random_username()
    ...
    user_in = UserCreate(email=email, username=username, password=password)
    crud.create_user(session=db, user_create=user_in)
    # Different email but same username -> username-conflict branch.
...
def test_update_password_me(
    ...
    user_query = select(User).where(User.username == superuser_username())
    user_db = db.exec(user_query).first()
    assert user_db
...
def test_register_user_username_already_exists_error(client: TestClient) -> None:
    password = random_lower_string()
    full_name = random_lower_string()
    data = {
        "email": random_email(),
